...
static core::smart_refctd_ptr<ICPUBuffer> computeLuminancePdf(smart_refctd_ptr<ICPUImage> envmap, float* normalizationFactor)
{
	...
	core::smart_refctd_ptr<ICPUBuffer> outBuffer = ICPUBuffer::create({ .size = outBufferSize });
...
public:
	...
	void onAppInitialized_impl() override
	{
		...
		CommonAPI::InitWithDefaultExt(initOutput, video::EAT_OPENGL_ES, "ImportanceSamplingEnvMaps", FRAMES_IN_FLIGHT, WIN_W, WIN_H, SC_IMG_COUNT, swapchainImageUsage, surfaceFormat, nbl::asset::EF_D32_SFLOAT);
...

		{
			...
			core::smart_refctd_ptr<ICPUBuffer> conditionalIntegrals = nullptr;
			{
				...
				core::smart_refctd_ptr<ICPUBuffer> conditionalCdfBuffer = ICPUBuffer::create({ .size = conditionalCdfBufferSize });
				...
				conditionalIntegrals = ICPUBuffer::create({ .size = pdfDomainExtent.Y * sizeof(double) });
			...
			double marginalIntegral = 0.0;
			{
				...
				// Create out image
				core::smart_refctd_ptr<ICPUBuffer> marginalCdfBuffer = ICPUBuffer::create({ .size = conditionalIntegrals->getSize() });
			...
			core::smart_refctd_ptr<ICPUBuffer> phiPdfLUTBuffer = ICPUBuffer::create({ .size = phiPdfLUTBufferSize });
			...
			core::smart_refctd_ptr<ICPUBuffer> thetaLUTBuffer = ICPUBuffer::create({ .size = thetaLUTBufferSize });
		...
		{
			logicalDevice->createCommandBuffers(commandPools[CommonAPI::InitOutput::EQT_GRAPHICS][i].get(), video::IGPUCommandBuffer::EL_PRIMARY, 1, commandBuffers+i);
