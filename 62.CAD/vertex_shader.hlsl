...
inline float2 quadraticDerivative(float t, float2 A, float2 B, float2 C)
{
    return 2.0f*A*t + B;
}

inline float2 quadraticSecDerivative(float t, float2 A, float2 B, float2 C)
{
    return 2.0f*A;
}
...
PSInput main(uint vertexID : SV_VertexID)
{
    ...
    else if (objType == ObjectType::QUAD_BEZIER)
    {
        ...
        outV.setBezierCurveTotalArcLen(preCompData.calcArcLen(1.0f));
