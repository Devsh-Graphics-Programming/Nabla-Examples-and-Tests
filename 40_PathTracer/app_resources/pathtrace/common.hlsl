...
SPixelSamplingInfo advanceSampleCount(const uint16_t3 coord, const uint16_t newSamplesThisPixel, const uint16_t dontClear, const uint32_t maxSamples)
{
	SPixelSamplingInfo retval;
	//
    const uint32_t oldSampleCount = gSampleCount[coord];
	...
	//
	// Clamp to >= firstSample so lowering maxSamples below the already-accumulated
	// count can't underflow samplesThisFrame (which would spin the per-sample loop until TDR).
	retval.newSampleCount = hlsl::max(retval.firstSample, hlsl::min(retval.firstSample+newSamplesThisPixel,maxSamples));
...
// ReSTIR stuff
// moved to renderer/shaders/pathtrace/resampling.hlsl
// TODO: should move these functions?
...
// TODO: this probably needs adjusting for orthographic projections, esp. calculating cellSizeStep
...
{
    const float32_t cellSizeStep = hlsl::length(pos - cameraPos) * hlsl::tan(120.f * params.vfov * hlsl::max(1.f / float(renderSize.y), float(renderSize.y) / (float(renderSize.x) * float(renderSize.x))));
    const float32_t logStep = hlsl::floor(hlsl::log2(cellSizeStep / params.minCellSize));
...
#endif // _NBL_THIS_EXAMPLE_APP_PATHTRACE_COMMON_HLSL_INCLUDED_