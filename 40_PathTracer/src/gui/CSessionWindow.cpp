...
void CSessionWindow::setSession(CSession* session)
{
   ...
   if (m_session)
   {
      ...
      m_state.tMin         = dynamics.nearClip;
...
void CSessionWindow::drawRenderModeSection()
{
   if (ImGui::CollapsingHeader("Render Mode", ImGuiTreeNodeFlags_DefaultOpen))
   {
      ...
      // Runs the bench inline on the GUI thread, so the UI freezes for the targetBudgetMs window.
...
void CSessionWindow::drawDynamicsSection()
{
   if (ImGui::CollapsingHeader("Dynamics", ImGuiTreeNodeFlags_DefaultOpen))
   {
      ...
      if (ImGui::DragFloat("Crop Offset X", &m_state.cropOffsetX, 1.0f, 0.0f, 10000.0f))
         changed = true;
      if (ImGui::DragFloat("Crop Offset Y", &m_state.cropOffsetY, 1.0f, 0.0f, 10000.0f))
         changed = true;
      if (ImGui::SliderFloat("T Min", &m_state.tMin, 0.0f, 20000.0f, "%.1f", ImGuiSliderFlags_Logarithmic))
         changed = true;
      ...
      // Bounded by the lastPathDepth bitfield width. On its own callback, not onDynamicsChanged,
      // because it must restart accumulation while maxSPP must not.
      ...
      if (changed && m_callbacks.onDynamicsChanged)
      {
         ...
         newDynamics.nearClip            = m_state.tMin;
...
struct to_string_helper<this_example::gui::CSessionWindow::BufferType>
{
   static std::string __call(const this_example::gui::CSessionWindow::BufferType& value)
   {
      switch (value)
      {
         case this_example::gui::CSessionWindow::BufferType::Beauty:
            return "Beauty";
         case this_example::gui::CSessionWindow::BufferType::Albedo:
            return "Albedo";
         case this_example::gui::CSessionWindow::BufferType::Normal:
            return "Normal";
         case this_example::gui::CSessionWindow::BufferType::Motion:
            return "Motion";
         case this_example::gui::CSessionWindow::BufferType::Mask:
            return "Mask";
         case this_example::gui::CSessionWindow::BufferType::RWMCCascades:
            return "RWMC Cascades";
         case this_example::gui::CSessionWindow::BufferType::SampleCount:
            return "Sample Count";
         default:
            return "Unknown";
      }
