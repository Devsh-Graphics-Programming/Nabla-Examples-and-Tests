...
// Layout: 2-binding descriptor set (binding 0 zero-filled and unused by the shader,
// binding 1 RWByteAddressBuffer for one uint32 per thread). 
	...
	void setup(const SetupData& data)
	{
		...
		video::IGPUDescriptorSetLayout::SBinding bindings[2] = {
			{ .binding = 0, .type = asset::IDescriptor::E_TYPE::ET_STORAGE_BUFFER,
			  ...
			  .stageFlags = hlsl::ShaderStage::ESS_COMPUTE, .count = 1 },
