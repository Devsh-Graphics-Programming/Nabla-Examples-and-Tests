...
#include "nbl/video/utilities/CGPUMeshPackerV2.h"
...
class VisibilityBufferApp : public ApplicationBase
{
...
public:
    ...
    APP_CONSTRUCTOR(VisibilityBufferApp)
    void onAppInitialized_impl() override
    {
        ...
        CommonAPI::InitWithDefaultExt(initOutput, video::EAT_VULKAN, "VisibilityBuffer", WIN_W, WIN_H, SC_IMG_COUNT, swapchainImageUsage, surfaceFormat, nbl::asset::EF_D32_SFLOAT);
        ...
        //
        auto createScreenSizedImage = [this](const E_FORMAT format) -> auto
        {
            IGPUImage::SCreationParams param;
            param.flags = static_cast<IImage::E_CREATE_FLAGS>(0u);
            param.type = IImage::ET_2D;
            param.format = format;
            param.extent = { WIN_W,WIN_H,1u };
            param.mipLevels = 1u;
            param.arrayLayers = 1u;
            param.samples = IImage::ESCF_1_BIT;
            return logicalDevice->createDeviceLocalGPUImageOnDedMem(std::move(param));
        };
        auto framebuffer = createScreenSizedImage(EF_R8G8B8A8_SRGB);
        auto createImageView = [this](smart_refctd_ptr<IGPUImage>&& image, const E_FORMAT format = EF_UNKNOWN) -> auto
        {
            ...
            return logicalDevice->createGPUImageView(std::move(params));
        };
        ...
        auto visBuffer = driver->addFrameBuffer(); // TODO
        ...
        //config.lvlLimit = 5u;
        DPG dpg(logicalDevice.get(), assetManager, depthBufferView, config);
...
#endif
        {
            ...
            auto mesh_raw = static_cast<asset::ICPUMesh*>(cpuMesh.get());
            ...
            {
                {
                    ...
                    bindings[0].stageFlags = IShader::ESS_VERTEX;
            ...
            smart_refctd_ptr<IGPUDescriptorSetLayout> vtDSLayout;
            {
                ...
                for (auto& binding : vtBindings)
                    binding.stageFlags = static_cast<IShader::E_SHADER_STAGE>(IShader::ESS_COMPUTE | IShader::ESS_FRAGMENT);
            ...
            [&](auto& layoutParams) -> void
            {
                ...
                for (auto& binding : vgBindings)
                    binding.stageFlags = static_cast<IShader::E_SHADER_STAGE>(IShader::ESS_VERTEX | IShader::ESS_COMPUTE | IShader::ESS_FRAGMENT);
            ...
            auto overrideShaderJustAfterVersionDirective = [this, &extraCode](const char* path)
            {
                asset::IAssetLoader::SAssetLoadParams lp;
                auto _specShader = IAsset::castDown<ICPUSpecializedShader>(*assetManager->getAsset(path, lp).getContents().begin());
                assert(_specShader);
            ...
            //
            {
                ...
                pcRange.offset = 0u;
                pcRange.stageFlags = IShader::ESS_VERTEX;
        ...
        // occlusion cull shader pipeline setup
        {
            ...
            asset::IAssetLoader::SAssetLoadParams lp;
            auto vtxOcclusionShader = IAsset::castDown<ICPUSpecializedShader>(*assetManager->getAsset("../occlusionCull.vert", lp).getContents().begin());
            auto fragOcclusionShader = IAsset::castDown<ICPUSpecializedShader>(*assetManager->getAsset("../occlusionCull.frag", lp).getContents().begin());
        ...
        {
            SPushConstantRange range{ IShader::ESS_COMPUTE,0u,sizeof(uint32_t) };
            ...
            asset::IAssetLoader::SAssetLoadParams lp;
            auto mapShader = IAsset::castDown<ICPUSpecializedShader>(*assetManager->getAsset("../occlusionCullMap.comp", lp).getContents().begin());
            assert(mapShader);
        ...
        {
            SPushConstantRange range{ IShader::ESS_COMPUTE,0u,sizeof(CullShaderData_t) };
            ...
            asset::IAssetLoader::SAssetLoadParams lp;
            auto cullShader = IAsset::castDown<ICPUSpecializedShader>(*assetManager->getAsset("../cull.comp", lp).getContents().begin());
            assert(cullShader);
        ...
        auto cullBatches = [this, &cullShaderData, &sceneData](const core::matrix4SIMD& vp, const core::vector3df& camPos, bool freezeCulling)
        {
        ...
        auto fillVBuffer = [this, &sceneData, &vBuffDs](core::smart_refctd_ptr<IGPUBuffer> mdiBuffer)
        {
            ...
            {
                driver->pushConstants(sceneData.fillVBufferPpln->getLayout(), IShader::ESS_ALL, 0u, sizeof(uint32_t), &sceneData.pushConstantsData[i]);
        ...
        matrix4SIMD projectionMatrix = matrix4SIMD::buildProjectionMatrixPerspectiveFovLH(core::radians(60.0f), float(WIN_W) / WIN_H, 0.1f, 5000.0f);
...
NBL_COMMON_API_MAIN(VisibilityBufferApp)
