...
class MeshLoadersWindowedApplication : public virtual SimpleWindowedApplication
{
...
public:
    ...
    MeshLoadersWindowedApplication(const hlsl::uint16_t2 initialResolution, const asset::E_FORMAT depthFormat, Args&&... args)
        : base_t(std::forward<Args>(args)...), m_initialResolution(initialResolution), m_depthFormat(depthFormat) {}

    core::vector<video::SPhysicalDeviceFilter::SurfaceCompatibility> getSurfaces() const override final
    {
        if (!m_surface)
        {
            ...
            params.windowCaption = "MeshLoaders";
    ...
    bool onAppInitialized(core::smart_refctd_ptr<system::ISystem>&& system) override
    {
