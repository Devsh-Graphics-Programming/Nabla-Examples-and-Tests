...
float32_t3 octahedronUVToDir(float64_t2 uv)
{
	...
	{
		float64_t rDegree = pc.zAngleDegreeRotation;

		const float32_t zAngleRadians = float32_t(rDegree * M_PI / 180.0);
		const float64_t cosineV = cos(zAngleRadians);
		const float64_t sineV = sin(zAngleRadians);
...
uint32_t implGetVUB(const float64_t angle)
{
...
uint32_t implGetHUB(const float64_t angle)
{
...
uint32_t getSymmetry() // TODO: to reduce check time we could pass it with PCs
{
	...
	const float64_t hABack = pc.getHorizontalAngle(pc.hAnglesCount - 1);
...
float64_t sampleI(const float32_t2 sphericalCoordinates, const uint32_t symmetry)
{
	const float64_t vAngle = degrees(sphericalCoordinates.x), hAngle = degrees(wrapPhi(sphericalCoordinates.y, symmetry));

	float64_t vABack = pc.getVerticalAngle(pc.vAnglesCount - 1);
	float64_t hABack = pc.getHorizontalAngle(pc.hAnglesCount - 1);
	...
	float64_t uReciprocal = i1 == i0 ? 1.0 : 1.0 / (pc.getHorizontalAngle(i1) - pc.getHorizontalAngle(i0));
	float64_t vReciprocal = j1 == j0 ? 1.0 : 1.0 / (pc.getVerticalAngle(j1) - pc.getVerticalAngle(j0));

	float64_t u = (hAngle - pc.getHorizontalAngle(i0)) * uReciprocal;
	float64_t v = (vAngle - pc.getVerticalAngle(j0)) * vReciprocal;

	float64_t s0 = getValue(i0, j0) * (1.0 - v) + getValue(i0, j1) * (v);
	float64_t s1 = getValue(i1, j0) * (1.0 - v) + getValue(i1, j1) * (v);
...
bool isWithinSCDomain(const float64_t2 p)
{
	const float64_t2 lb = float64_t2(0, -M_PI);
	const float64_t2 ub = float64_t2(M_PI, M_PI);
...
void main(uint32_t3 ID : SV_DispatchThreadID)
{
	...
	if (all(pixelCoordinates < destinationSize))
	{
		...
		outIESCandelaImage[pc.texIx][pixelCoordinates] = float32_t(sampleI(sphericalCoordinates, getSymmetry()) / pc.maxIValue);
		outSphericalCoordinatesImage[pc.texIx][pixelCoordinates] = sphericalCoordinates;
		outOUVProjectionDirectionImage[pc.texIx][pixelCoordinates] = direction;
