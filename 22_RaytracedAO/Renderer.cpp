				...
				// virtually allocate and size the storage
				{
					...
					for (const auto& asset : contents)
					{
						...
						for (auto mbIt=meshBuffers.begin(); mbIt!=meshBuffers.end(); mbIt++)
						{
							...
							auto newBuff = ICPUBuffer::create({ .size = sizeof(CombinedNormalUV)*approxVxCount });
					...
					newInstanceDataBuffer = ICPUBuffer::create({ .size = sizeof(ext::MitsubaLoader::instance_data_t)*batchInstanceBoundTotal });
				}
...
core::smart_refctd_ptr<asset::ICPUBuffer> Renderer::SampleSequence::createCPUBuffer(uint32_t quantizedDimensions, uint32_t sampleCount)
{
	...
	if (bytesize)
		return asset::ICPUBuffer::create({ .size = bytesize });
	else
		return nullptr;
}
