...
constexpr std::string_view BatchOptionShort = "-batch";
constexpr std::string_view BatchOptionLong = "--batch";
...
bool isBatchOption(std::string_view token)
{
	return token==BatchOptionShort || token==BatchOptionLong;
}

std::optional<std::string_view> mapLegacyOption(std::string_view key)
{
	if (key=="COLOR_FILE")
		return "--color-file";
	if (key=="ALBEDO_FILE")
...
std::vector<std::string> splitBatchLine(const std::string& line, std::string& errorMessage)
{
...
std::vector<std::vector<std::string>> loadBatchInvocations(const std::filesystem::path& batchFile, std::string& errorMessage)
{
	...
	while (std::getline(input,line))
	{
		...
		std::vector<std::string> invocation = {"39_denoisertonemapper"};
...
std::vector<std::string> normalizeInvocationArguments(const std::vector<std::string>& rawArguments, std::string& errorMessage)
{
	...
	for (size_t i = 1u; i < rawArguments.size(); ++i)
	{
		...
		if (token[0]=='-')
		{
			...
			if (equalsPos==std::string::npos)
			{
				errorMessage = "Legacy option \""+token+"\" must use the form -NAME=value.";
				return {};
			}

			const auto legacyName = token.substr(1u,equalsPos-1u);
			const auto normalizedOption = mapLegacyOption(legacyName);
			if (!normalizedOption.has_value())
			{
				errorMessage = "Unknown legacy option \""+legacyName+"\".";
				return {};
			}
...
std::optional<DenoiserTonemapperInvocation> parseSingleInvocation(const std::vector<std::string>& rawArguments, std::string& errorMessage)
{
	auto normalizedArguments = normalizeInvocationArguments(rawArguments,errorMessage);
	if (normalizedArguments.empty())
		return std::nullopt;

	argparse::ArgumentParser parser("39_denoisertonemapper");
	parser.add_description("Shared CLI parser for the denoiser tonemapper and its stub mode.");
...
std::optional<std::filesystem::path> tryGetInlineBatchPath(std::string_view token)
{
	...
	std::string& errorMessage)
{
	...
	if (isBatchOption(arguments[1]))
	{
		...
		rawInvocations = loadBatchInvocations(stripWrappingQuotes(arguments[2]),errorMessage);
	}
	else if (const auto batchFile = tryGetInlineBatchPath(arguments[1]); batchFile.has_value())
	{
		...
		rawInvocations = loadBatchInvocations(batchFile.value(),errorMessage);
	}
	else
	{
	...
	{
		auto invocation = parseSingleInvocation(rawInvocations[i],errorMessage);
