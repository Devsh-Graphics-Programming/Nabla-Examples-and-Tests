...
#ifndef _S_CAMERA_TOOLING_THRESHOLDS_HPP_
#define _S_CAMERA_TOOLING_THRESHOLDS_HPP_
...
struct SCameraToolingThresholds final
{
    ...
    static inline constexpr nbl::hlsl::float64_t ScalarTolerance = 1e-6;
    /// @brief Small epsilon used by replay and comparison helpers that need stricter zero tests.
    static inline constexpr nbl::hlsl::float64_t TinyScalarEpsilon = 1e-9;
    /// @brief Default world-space position tolerance used by pose comparisons.
    static inline constexpr nbl::hlsl::float64_t DefaultPositionTolerance = 2.0 * ScalarTolerance;
    /// @brief Default angular tolerance in degrees used by pose and state comparisons.
    static inline constexpr nbl::hlsl::float64_t DefaultAngularToleranceDeg = 1e-1;
};

#endif // _S_CAMERA_TOOLING_THRESHOLDS_HPP_
