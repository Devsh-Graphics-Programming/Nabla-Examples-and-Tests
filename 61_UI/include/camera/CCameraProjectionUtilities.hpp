...
struct CCameraProjectionUtilities final
{
    ...
    static inline bool syncDynamicPerspectiveProjection(nbl::ext::cameras::ICamera* camera, nbl::ext::cameras::CPlanarProjection& projection)
    {
        if (!camera)
            return false;

        const auto& params = projection.getParameters();
        if (params.kind != nbl::ext::cameras::CPlanarProjection::EKind::Perspective)
            return false;
        ...
        projection.setPerspective(params.zNear, params.zFar, dynamicFov);
        return true;
    }
};
