		...
		inline void workLoopBody() override
		{
			...
			auto execute = [&](E_IMAGE_REGIONS mode) -> bool
			{
				...
				auto getRespecifedView = [&]() -> core::smart_refctd_ptr<ICPUImageView>
				{
					...
					smart_refctd_ptr<ICPUBuffer> inBuffer = asset::ICPUBuffer::create({ .size = inImage->getBuffer()->getSize(), .data = (uint8_t*)inImage->getBuffer()->getPointer(), .memoryResource = core::getNullMemoryResource() }, core::adopt_memory); // adopt memory & don't free it on exit
