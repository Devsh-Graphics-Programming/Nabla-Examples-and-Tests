...
static const uint XPrevMaterialMask = 0x0000000cu;
...
{
    cellMaterials = (cellMaterials & ~XPrevMaterialMask) | ((cellMaterial << XPrevMatMaskShift) & XPrevMaterialMask)
}
inline uint getXPrevMaterial(uint cellMaterials)
{
    return (cellMaterials & XPrevMaterialMask) >> XPrevMatMaskShift;
}
...
{
    cellMaterials = (cellMaterials & ~YPrevMaterialMask) | ((cellMaterial << YPrevMatMaskShift) & YPrevMaterialMask)
}
inline uint getYPrevMaterial(uint cellMaterials)
{
    return (cellMaterials & YPrevMaterialMask) >> YPrevMatMaskShift;
}
...
{
    cellMaterials = (cellMaterials & ~ZPrevMaterialMask) | ((cellMaterial << ZPrevMatMaskShift) & ZPrevMaterialMask)
}
inline uint getZPrevMaterial(uint cellMaterials)
{
    return (cellMaterials & ZPrevMaterialMask) >> ZPrevMatMaskShift;
}
...
{
    bool3 is_solid_cell =
        (bool3)isSolidCell(getCellMaterial(cellMaterial)) ||
...
#define LOOP_NEIGHBOR_CELLS_BEGIN(CELL_IDX, NEIGHBOR_IDX, N_ID, RANGE) {\
...
for (int k = (int)CELL_IDX.z + RANGE[4]; k <= (int)CELL_IDX.z + RANGE[5]; ++k) {\
    ...
    const uint N_ID = CellIndexToCellID(NEIGHBOR_IDX);\
