	...
	in vec3 origin, in vec3 prevThroughput, in float prevAoVThroughputScale, inout vec3 albedo, out vec3 worldspaceNormal)
{
    ...
    float origin_offset = nbl_glsl_numeric_limits_float_epsilon(1u<<15u); // I pulled the constants out of my @$$
    origin_offset += dot(abs(ray_offset_vector),abs(origin))*nbl_glsl_numeric_limits_float_epsilon(1u<<9u);
