...
class OBBApp : public ApplicationBase
{
...
public:
    ...
    core::matrix3x4SIMD* TransMat;
    APP_CONSTRUCTOR(OBBApp)
        void onAppInitialized_impl() override
    {
        ...
        m_draw3DLine = ext::DebugDraw::CDraw3DLine::create(logicalDevice);
        ...
        TransMat = new core::matrix3x4SIMD[meshRaw->getMeshBuffers().size()];
        ...
        for (size_t i = 0ull; i < meshRaw->getMeshBuffers().size(); ++i)
        {
            ...
            TransMat[i] = nbl::asset::IMeshManipulator::calculateOBB(meshBuffer);
        }
...

        {
...

            {
                ...
                m_pipeline = logicalDevice->createGraphicsPipeline(nullptr, std::move(gp_params));

                core::vectorSIMDf Verts[8];
                core::vectorSIMDf Col = core::vectorSIMDf(1, 0, 0, 1);
                for (int i3 = 0; i3 < meshRaw->getMeshBuffers().size(); i3++) {
                    int Index = i3 % 10;
                    for (int i = 2; i >= 0; i--) {
                        Col[i] = Index % 2;
                        Index /= 2;
                    }
                    core::vectorSIMDf V0 = core::vectorSIMDf(0, 0, 0); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[0] = V0;
                    V0 = core::vectorSIMDf(1, 0, 0); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[1] = V0;
                    V0 = core::vectorSIMDf(1, 0, 1); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[2] = V0;
                    V0 = core::vectorSIMDf(0, 0, 1); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[3] = V0;
                    V0 = core::vectorSIMDf(1, 1, 1); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[4] = V0;
                    V0 = core::vectorSIMDf(0, 1, 1); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[5] = V0;
                    V0 = core::vectorSIMDf(0, 1, 0); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[6] = V0;
                    V0 = core::vectorSIMDf(1, 1, 0); TransMat[i3].transformVect(V0); V0 -= TransMat[i3].getTranslation(); Verts[7] = V0;
                    AddTriangle(Verts[0], Verts[1], Verts[2], Col);
                    AddTriangle(Verts[0], Verts[2], Verts[3], Col);
                    AddTriangle(Verts[6], Verts[5], Verts[4], Col);
                    AddTriangle(Verts[6], Verts[4], Verts[7], Col);
                    AddTriangle(Verts[1], Verts[7], Verts[4], Col);
                    AddTriangle(Verts[1], Verts[4], Verts[2], Col);
                    AddTriangle(Verts[0], Verts[3], Verts[5], Col);
                    AddTriangle(Verts[0], Verts[5], Verts[6], Col);
                    AddTriangle(Verts[0], Verts[6], Verts[7], Col);
                    AddTriangle(Verts[0], Verts[7], Verts[1], Col);
                    AddTriangle(Verts[2], Verts[4], Verts[5], Col);
                    AddTriangle(Verts[2], Verts[5], Verts[3], Col);

                }
                m_draw3DLine->setLinesData(lines);
                auto fence = logicalDevice->createFence(static_cast<video::IGPUFence::E_CREATE_FLAGS>(0));
                m_draw3DLine->updateVertexBuffer(utilities.get(), queues[CommonAPI::InitOutput::EQT_GRAPHICS], &fence);
                logicalDevice->waitForFences(1, const_cast<video::IGPUFence**>(&fence.get()), false, MAX_TIMEOUT);
    ...
    void AddTriangle(const core::vectorSIMDf V0, const core::vectorSIMDf V1, const core::vectorSIMDf V2, const core::vectorSIMDf Col) {
        lines.push_back({{
            {V0.x,V0.y,V0.z},     // start origin
            {Col.x,Col.y,Col.z,Col.w} // start color
            }, {
            {V1.x,V1.y,V1.z}, // end origin
            {Col.x,Col.y,Col.z,Col.w}         // end 
            }});
        lines.push_back({{
            {V2.x,V2.y,V2.z},     // start origin
            {Col.x,Col.y,Col.z,Col.w} // start color
            }, {
            {V1.x,V1.y,V1.z}, // end origin
            {Col.x,Col.y,Col.z,Col.w}         // end 
    ...
    void workLoopBody() override
    {
        ...
        m_draw3DLine->setViewProjMatrix(viewProjectionMatrix);
        ...
        m_draw3DLine->recordToCommandBuffer(commandBuffer.get(), m_pipeline.get());
